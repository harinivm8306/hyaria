...
def train_harvest_model(csv_path):
    ...
    save_path = os.path.join(os.path.dirname(__file__), 'harvest_model.pkl')
    with open(save_path, 'wb') as f:
        pickle.dump({'model': model, 'scaler': scaler}, f)
    print(f"Harvest model saved to {save_path}")

if __name__ == "__main__":
    csv_file = os.path.join(os.path.dirname(__file__), '..', 'hyaria_dataset.csv')
    train_harvest_model(csv_file)
